...
public:
    ...
    /**
     * @brief Holds the geometric and steering information for a single fine‑in‑beam
     * detection element.
     */

    struct DETECT_CORE_API InBeamObject
    {   
        /// @brief look/ pulse identifier
        unsigned int lookId;          
        /// @brief Direction‑cosine U (cross‑range)
        double u;    
        /// @brief Direction‑cosine V (elevation)
        double v;                    
        /// @brief Slant‑range (meters)
        double r;                    
        /// @brief Receive‑steering U component
        double rxSteerU;      
        /// @brief Receive‑steering V component
        double rxSteerV;             
        /// @brief Start range of the beam segment (m)
        double rangeStart;  
         /// @brief End range of the beam segment (m)         
        double rangeStop;            
        /// @brief Offset from beam centre in U
        double deltaU;    
        /// @brief Offset from beam centre in V            
        double deltaV;                
        /// @brief squared distance in U‑space
        double distanceSqdU;      
        /// @brief squared distance in V‑space
        double distanceSqdV;         
        /// @brief Effective beam‑radius in U‑space
        double effBeamRadSqdU;       
        /// @brief Effective beam‑radius in V‑space    
        double effBeamRadSqdV;       
        /// @brief Combined angular error metric
        double AngleError;           
        /// @brief Ground‑truth object identifier
    ...
    //Beam Object
    void setInBeamObject(const InBeamObject& inbeam_obj);
    const InBeamObject& inbeamObject() const;  
...
private:
    ...
    /// @brief The object in the inbeam that we will asseess for inbeam algorithm
    InBeamObject beam_object_;
